...
class Attributes : Iterable<Attribute?> {
    ...
    var vals = arrayOfNulls<String>(InitialCapacity)
    ...
    fun html(accum: Appendable, out: OutputSettings) {
        val sz = size
        for (i in 0 until sz) {
            if (isInternalKey(keys[i])) continue
            val key = Attribute.getValidKey(keys[i], out.syntax())
            if (key != null) Attribute.htmlNoValidate(key, vals[i], accum.append(' '), out)
        }
    }
    ...
    fun clone(): Attributes {
        ...
        clone.size = size
        clone.keys = keys.copyOf(size)
        clone.vals = vals.copyOf(size)
        return clone
    }
    ...
    private class Dataset(private val attributes: Attributes) : AbstractMutableMap<String, String?>() {
        ...
        private inner class EntrySet : AbstractMutableSet<MutableEntry<String, String?>>() {
        ...
        private inner class DatasetIterator : MutableIterator<MutableMap.MutableEntry<String, String?>> {
            ...
            private lateinit var attr: Attribute
            override fun hasNext(): Boolean {
